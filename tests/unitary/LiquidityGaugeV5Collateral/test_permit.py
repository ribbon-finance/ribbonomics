...
def test_permit(accounts, bob, chain, gauge_v5_collateral):

    alice = accounts.add("0x416b8a7d9290502f5661da81f0cf43893e3d19cb9aea3c426cfb36e8186e9c09")

    class Permit(EIP712Message):
        # EIP-712 Domain Fields
        _name_: "string" = gauge_v5_collateral.name()  # noqa: F821
        _version_: "string" = gauge_v5_collateral.version()  # noqa: F821
        _chainId_: "uint256" = chain.id  # noqa: F821
        _verifyingContract_: "address" = gauge_v5_collateral.address  # noqa: F821
    ...
    tx = gauge_v5_collateral.permit(alice, bob, 2 ** 256 - 1, 2 ** 256 - 1, sig.v, sig.r, sig.s)

    assert gauge_v5_collateral.allowance(alice, bob) == 2 ** 256 - 1
    assert tx.return_value is True
    assert len(tx.events) == 1
    assert tx.events["Approval"].values() == [alice.address, bob, 2 ** 256 - 1]
    assert gauge_v5_collateral.nonces(alice) == 1


def test_permit_contract(accounts, bob, chain, gauge_v5_collateral):
...
def isValidSignature(_hash: bytes32, _sig: Bytes[65]) -> bytes32:
    ...
    tx = gauge_v5_collateral.permit(mock_contract, bob, 2 ** 256 - 1, 2 ** 256 - 1, sig.v, sig.r, sig.s)
