...
class StateMachine:
    ...
    def __init__(self, accounts, gauge_v5_collateral, mock_lp_token):
        self.accounts = accounts
        self.token = mock_lp_token
        self.gauge_v5_collateral = gauge_v5_collateral
    ...
    def rule_deposit(self, st_account, st_value):
        ...
        self.gauge_v5_collateral.deposit(st_value, {"from": st_account})
    ...
    def rule_withdraw(self, st_account, st_value):
        ...
        if self.balances[st_account] < st_value:
            ...
            with brownie.reverts():
                self.gauge_v5_collateral.withdraw(st_value, {"from": st_account})
            return
    ...
    def rule_checkpoint(self, st_account):
        ...
        """
        self.gauge_v5_collateral.user_checkpoint(st_account, {"from": st_account})
    ...
    def invariant_balances(self):
        ...
        for account, balance in self.balances.items():
            assert self.gauge_v5_collateral.balanceOf(account) == balance

    def invariant_total_supply(self):
        """
        Validate expected total supply against actual total supply.
        """
        assert self.gauge_v5_collateral.totalSupply() == sum(self.balances.values())
    ...
    def teardown(self):
        ...
        for account, balance in ((k, v) for k, v in self.balances.items() if v):
            ...
            self.gauge_v5_collateral.withdraw(balance, {"from": account})

            assert self.token.balanceOf(account) == initial + balance


def test_state_machine(state_machine, accounts, gauge_v5_collateral, mock_lp_token, no_call_coverage):
    # fund accounts to be used in the test
    for acct in accounts[1:5]:
        mock_lp_token.transfer(acct, 10 ** 21, {"from": accounts[0]})

    # approve gauge_v5_collateral from the funded accounts
    for acct in accounts[:5]:
        mock_lp_token.approve(gauge_v5_collateral, 2 ** 256 - 1, {"from": acct})

    # because this is a simple state machine, we use more steps than normal
    settings = {"stateful_step_count": 25, "max_examples": 30}

    state_machine(StateMachine, accounts[:5], gauge_v5_collateral, mock_lp_token, settings=settings)
