...
def test_gauge_integral(accounts, chain, mock_lp_token, token, gauge_v5_collateral, minter, gauge_controller):
    ...
    gauge_controller.add_gauge(gauge_v5_collateral.address, 0, 10 ** 18, {"from": alice})
    ...
    def update_integral():
        ...
        checkpoint = t1
        checkpoint_supply = gauge_v5_collateral.totalSupply()
        checkpoint_balance = gauge_v5_collateral.balanceOf(alice)
    ...
    for i in range(40):
        ...
        if is_withdraw:
            amount = randrange(1, gauge_v5_collateral.balanceOf(bob) + 1)
            gauge_v5_collateral.withdraw(amount, {"from": bob})
            update_integral()
            bob_staked -= amount
        else:
            amount = randrange(1, mock_lp_token.balanceOf(bob) // 10 + 1)
            mock_lp_token.approve(gauge_v5_collateral.address, amount, {"from": bob})
            gauge_v5_collateral.deposit(amount, {"from": bob})
            update_integral()
            bob_staked += amount

        if is_alice:
            # For Alice
            is_withdraw_alice = (gauge_v5_collateral.balanceOf(alice) > 0) * (random() < 0.5)
            print("Alice", "withdraws" if is_withdraw_alice else "deposits")

            if is_withdraw_alice:
                amount_alice = randrange(1, gauge_v5_collateral.balanceOf(alice) // 10 + 1)
                gauge_v5_collateral.withdraw(amount_alice, {"from": alice})
                update_integral()
                alice_staked -= amount_alice
            else:
                amount_alice = randrange(1, mock_lp_token.balanceOf(alice) + 1)
                mock_lp_token.approve(gauge_v5_collateral.address, amount_alice, {"from": alice})
                gauge_v5_collateral.deposit(amount_alice, {"from": alice})
                update_integral()
                alice_staked += amount_alice
        ...
        if random() < 0.5:
            gauge_v5_collateral.user_checkpoint(alice, {"from": alice})
        if random() < 0.5:
            gauge_v5_collateral.user_checkpoint(bob, {"from": bob})

        assert gauge_v5_collateral.balanceOf(alice) == alice_staked
        assert gauge_v5_collateral.balanceOf(bob) == bob_staked
        assert gauge_v5_collateral.totalSupply() == alice_staked + bob_staked

        dt = randrange(WEEK, WEEK*3)
        chain.sleep(dt)
        chain.mine()
        ...
        print(i, dt / 86400, integral, gauge_v5_collateral.integrate_fraction(alice))

        assert approx(gauge_v5_collateral.integrate_fraction(alice), integral, 1e-12)

def test_gauge_integral_same_claimable(accounts, chain, mock_lp_token, token, gauge_v5_collateral, minter, gauge_controller):
    alice, bob = accounts[:2]
    ...
    for i in range(80):
        ...
        if i % 5 == 0:
            gauge_v5_collateral.user_checkpoint(alice, {"from": alice})
            print(i, dt / 86400, gauge_v5_collateral.integrate_fraction(bob), gauge_v5_collateral.integrate_fraction(alice))
            assert approx(gauge_v5_collateral.integrate_fraction(alice), gauge_v5_collateral.integrate_fraction(bob), 1e-5)


def test_gauge_integral_extra(accounts, chain, mock_lp_token, token, gauge_v5_collateral, minter, gauge_controller):
    alice, bob = accounts[:2]
    ...
    for i in range(40):
        ...
        if (i % 10 == 0):
            ...
            update_integral()
            print(i, dt / 86400, integral, gauge_v5_collateral.integrate_fraction(alice))
            assert approx(gauge_v5_collateral.integrate_fraction(alice), integral, 1)
        else:
            update_integral()
            gauge_v5_collateral.user_checkpoint(bob, {"from": bob})
...
def test_mining_with_votelock(
    ...
    mock_lp_token.transfer(bob, mock_lp_token.balanceOf(alice) // 2, {"from": alice})
    mock_lp_token.approve(gauge_v5_collateral.address, MAX_UINT256, {"from": alice})
    mock_lp_token.approve(gauge_v5_collateral.address, MAX_UINT256, {"from": bob})
    ...
    gauge_v5_collateral.deposit(10 ** 21, {"from": alice})
    gauge_v5_collateral.deposit(10 ** 21, {"from": bob})
    ...
    rewards_alice = gauge_v5_collateral.integrate_fraction(alice)
    rewards_bob = gauge_v5_collateral.integrate_fraction(bob)
